...
```
{
  ...
  "global_size": [6.16, 7.68, ..., 19.80],
  "regions": [{"name": "권역명 (≤20자)",
    "sizes_usd_b": [3.38, 4.30, ..., 10.76],
    "cagr_pct": 14.6,
...
`short_term_*`/`long_term_*`은 "권역별 기회 및 리스크" 표 셀에 그대로 들어가므로 반드시 **30자
이내**로 간결하게 작성한다 (넘으면 잘림).

### `years` / `global_size` / `regions[].sizes_usd_b` — 권역별 시장규모 그림(V4_1)용

**이 세 필드를 채워야 V4_1(권역별 시장규모 추이 선그래프)이 생성된다.** 하나라도 비면 `v4_1()`이
"지역별 데이터 없음"으로 그림을 통째로 건너뛴다 — 실제로 이 필드들이 문서화되지 않아 V4_1이 조용히
누락된 적이 있다(실측 확인).

- `years`: 원문 권역별 표의 연도 배열. 연도가 많아도 무방하다 — `v4_1()`은 선그래프로 그리고 수치
  라벨을 시작·끝점에만 단다.
- `global_size`: 전체(합계) 시장규모 시계열. 생략 가능하지만, 넣으면 권역들과 함께 한 계열로 그려져
  전체 대비 각 권역의 비중을 읽을 수 있다.
- `regions[].sizes_usd_b`: 그 권역의 시장규모 시계열. `years`와 **길이가 같아야** 한다(짧으면 그 권역이
  차트에서 빠진다). **차트에는 `regions`의 앞 4개 권역만 그려지므로, 규모가 큰 권역부터 순서대로 넣을 것**
  (5번째 이후 권역도 "권역별 기회 및 리스크" 표에는 정상적으로 모두 들어간다).
- `cagr_pct`: 그 권역의 CAGR. 본문 서술에 쓰인다.

**단위는 다른 `*_usd_b`/`sizes` 필드와 동일하게 항상 십억(B) 단위로 정규화**한다 — 원문이 USD Million
이면 1,000으로 나눠 넣을 것(예: 3,376.4 → 3.3764). 표시 통화 기호는 `sec1.currency_unit`이 결정한다.
...
```
{
  "patent_trend": {
    ...
    "top_countries_detail": [{"country": "국가1", "count": 12345}, ...],
...
### 표는 '국가별', 그림은 '기업별' — 두 축을 모두 채울 것

**특허 동향은 표와 그림을 서로 다른 축으로 분석한다:**

| 산출물 | 사용 필드 | 축 |
| --- | --- | --- |
| **표** (본문) | `top_countries_detail` | **국가별** 특허 보유(출원) 건수 |
| **그림** (로그스케일 막대그래프 V6_2) | `top_companies_detail` | **기업별** 특허 보유(출원) 건수 |

특허 경쟁 구도는 국가 단위(표)와 기업 단위(그림)를 함께 보여줄 때 가장 유용하므로, STEP 3에서
원문으로부터 **두 축의 수치를 모두** 추출해 `top_countries_detail`(국가별)과
`top_companies_detail`(기업별)을 각각 채울 것.

- 원문에 **국가별 수치**가 있으면 `top_countries_detail`에 `{"country": ..., "count": ...}`로 채운다
  → 표가 국가별로 그려진다. 없으면 표는 생략되고(수치를 지어내지 말 것) 국가명 목록만 불릿으로 남는다.
- 원문에 **기업별 수치**가 있으면 `top_companies_detail`에 `{"company": ..., "count": ...}`로 채운다
  → 그림(V6_2)이 기업별로 그려진다. 없으면 그림은 생략되고 기업명 목록만 불릿으로 남는다.

수치를 알 수 없으면 `top_countries`/`top_companies`(이름만)만 채우고 `*_detail`은 생략 — 없는 수치를
지어내지 말 것. `top_countries`/`top_companies`(이름만)는 항상 본문에 불릿으로 별도 표시되므로,
detail 수치가 없더라도 이름 목록만은 양쪽 다 채워둘 것.
